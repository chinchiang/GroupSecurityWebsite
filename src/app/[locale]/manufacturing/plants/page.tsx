...
import { listPlants } from "@/services/catalog";
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  let rows;
  try {
    rows = await listPlants(session.user);
  } catch (e) {
  ...
  return (
    <div>
      ...
      <PageHeader title={t(locale, "nav.plants")} />
      ...
      <DataTable
        ...
        columns={[
          { id: "name", header: "Name" },
          { id: "code", header: "Code" },
          { id: "risk", header: "OT risk" },
          { id: "uns", header: "Unsupported OT" },
          { id: "rec", header: "Recovery" }
        ]}
        rows={rows.map((r) => ({
          id: r.id,
          detailHref: localizedHref(locale, `/manufacturing/plants/` + r.id),
          cells: {
            name: r.name,
            code: r.code,
            risk: r.otRiskLevel,
            uns: String(r.unsupportedOtCount),
            rec: r.recoveryReadiness,
          },
        }))}
      />
    </div>
