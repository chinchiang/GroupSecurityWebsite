...
import { listRisks } from "@/services/catalog";
...
export default async function RisksPage({
  ...
  let rows;
  try {
    rows = await listRisks(session.user);
  } catch (e) {
  ...
  return (
    <div>
      ...
      <PageHeader title={t(locale, "nav.risks")} />
      <DataTable
        caption="Enterprise risks"
        ...
        rows={rows.map((r) => ({
          id: r.id,
          detailHref: localizedHref(locale, `/governance/risks/${r.id}`),
          cells: {
            title: r.title,
            level: r.riskLevel,
            owner: r.owner,
