...
import { listSuppliers } from "@/services/catalog";
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  let rows;
  try {
    rows = await listSuppliers(session.user);
  } catch (e) {
  ...
  return (
    <div>
      ...
      <PageHeader title={t(locale, "nav.supplyChain")} />
      ...
      <DataTable
        ...
        columns={[
          ...
          { id: "tier", header: "Tier" },
          { id: "risk", header: "Risk" },
          { id: "remote", header: "Remote access" }
        ]}
        rows={rows.map((r) => ({
          id: r.id,
          detailHref: localizedHref(locale, `/supply-chain/` + r.id),
          cells: {
            name: r.name,
            tier: r.tier,
            risk: r.riskLevel,
            remote: String(r.remoteAccess),
          },
        }))}
      />
    </div>
