...
import { getSupplier } from "@/services/catalog";
...
export default async function Page({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  let item;
  try { item = await getSupplier(session.user, id); }
  catch (e) {
  ...
  return (
    <div>
      ...
      <dl className="grid gap-3 rounded-md border border-slate-200 bg-white p-4 text-sm md:grid-cols-2">
        <div><dt className="font-semibold">Tier</dt><dd>{item.tier}</dd></div>
        <div><dt className="font-semibold">Risk</dt><dd><SeverityBadge level={item.riskLevel} locale={locale} /></dd></div>
        <div><dt className="font-semibold">Last assessment</dt><dd>{item.lastAssessmentDate}</dd></div>
        <div><dt className="font-semibold">Contract controls</dt><dd>{item.contractControlsStatus}</dd></div>
      </dl>
    </div>
  );
}
