...
import { listKnowledge } from "@/services/catalog";
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  let rows;
  try {
    rows = await listKnowledge(session.user);
  } catch (e) {
  ...
  return (
    <div>
      ...
      <PageHeader title={t(locale, "nav.knowledge")} />
      ...
      <DataTable
        ...
        rows={rows.map((r) => ({
          id: r.id,
          detailHref: localizedHref(locale, `/knowledge/` + r.id),
          cells: {
            title: r.metadata.title,
            cat: r.category,
            status: r.metadata.status,
            ver: r.metadata.version,
          },
        }))}
      />
    </div>
